...
select th_cinema from THEATER where th_name = '명동' group by th_cinema 
select th_num from theater where th_name='명동' and th_cinema='1관';
...
INSERT INTO SEAT VALUES (S_SEQ.NEXTVAL,1,'p10',0);
INSERT INTO SEAT VALUES (S_SEQ.NEXTVAL,1,'p9',0);
INSERT INTO SEAT VALUES (S_SEQ.NEXTVAL,1,'p9',0);
...
select * from exam;
select * from THEATER;
insert into exam values (TO_DATE('20160707', 'YYYYMMDD'))

SELECT * FROM (SELECT sub.*, ROWNUM AS RNUM FROM ( select * from review where r_title = 'zz' order by r_writedate desc) sub) WHERE RNUM >= 1 AND RNUM <= 10 
